...
# Fixed policy, matching ablation_study/hierarchical_rag/src/run.py's retrieval-only
# defaults -- ignores the Retrieval Planner's (k, max_hops, allowed_edges) entirely,
# same "no agentic planning" framing as the retrieval-only ablation.
_K_SECTIONS = 3
_K_CHILDREN = 5
...
def _adapter(client, settings, embedder, query, k=5, max_hops=2, allowed_edges=None, question=""):
    ...
    return hierarchical_retrieve(ctx, q, k_sections=_K_SECTIONS, k_children=_K_CHILDREN)
